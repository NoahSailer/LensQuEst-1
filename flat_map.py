...
#      lnfln = interp1d(np.log(self.l.flatten()), np.log(n0Kappa.flatten()), kind='linear', bounds_error=False, fill_value=0.)
   ...
   def computeResponseFFT(self, fC0, fCtot, lMin=1., lMax=1.e5, test=False, cache=None):
      '''calculate the response R_\ell'''
      ...
      def doCalculation():
         ...
         term1x *= iVar
         term1xFourier = self.fourier(data=term1x)
         term1xFourier *= self.lx / (self.lx**2. + self.ly**2.)
	 # correction for the artificial i in WFFourier
         ...
         term1y *= iVar 
         term1yFourier = self.fourier(data=term1y)
         term1yFourier *= self.ly / (self.lx**2. + self.ly**2.)
	 # correction for the artificial i in WFFourier
         term1yFourier /= 1.j
	 ...
	 #Normalization factor
      ...
      # if caching is desired
         ...
         # if the calculation has been done before
         if self.computeResponseFFT.cache.has_key(cache):
   ...
   def computeSNormalizationFFT(self, fCtot, lMin=1., lMax=1.e5, test=False, cache=None):
	 ...
	 # invert
      ...
      # if caching is desired
         ...
         # if the calculation has been done before
         if self.computeS2NormalizationFFT.cache.has_key(cache):
   ...
   def quadEstSNonNorm(self, fCtot, lMin= 1., lMax=1.e5, dataFourier=None, dataFourier2=None, test=False):
   ...
   def computeQuadEstSNorm(self, fCtot, lMin=1., lMax=1.e5, dataFourier=None, dataFourier2=None, path=None, test=False, cache=None):
      '''Returns the normalized quadratic estimator for S2 in Fourier space,
      and saves it to file if needed.
      '''
      # non-normalized QE for S2
      resultFourier = self.quadEstSNonNorm(fCtot, lMin=lMin, lMax=lMax, dataFourier=dataFourier, dataFourier2=dataFourier2, test=test)
      # compute normalization
      normalizationFourier = self.computeSNormalizationFFT(fCtot, lMin=lMin, lMax=lMax, test=test, cache=cache)
      # normalized (not mean field-subtracted) QE for kappa
      resultFourier *= normalizationFourier
      # save to file if needed
      if path is not None:
   ...
   def computeQuadEstKappaPointSourceHardenedNorm(self, fC0, fCtot, lMin=1., lMax=1.e5, dataFourier=None, dataFourier2=None, path=None, test=False, cache=None):
      ...
      SMinVarFourier = self.computeQuadEstSNorm(fCtot, lMin=lMin, lMax=lMax, dataFourier=dataFourier, dataFourier2=dataFourier2, test=test, cache=cache)
      ...
      NSFourier = self.computeSNormalizationFFT(fCtot, lMin=lMin, lMax=lMax, test=test, cache=cache)
      # response
      RFourier = self.computeResponseFFT(fC0, fCtot, lMin=lMin, lMax=lMax, test=test, cache=cache)
   ...
   def forecastN0S(self, fCtot, lMin=1., lMax=1.e5, test=False):
      """Interpolates the result for N_L^kappa = f(L),
      to be used for forecasts on lensing reconstruction
      """
      #print "computing the reconstruction noise"
      # Standard reconstruction noise
      n0S = self.computeSNormalizationFFT(fCtot=fCtot, lMin=lMin, lMax=lMax, test=test, cache=None)
      # keep only the real part (the imag. part should be zero, but is tiny in practice)
      n0S = np.real(n0S)
      # remove the nans
   ...
   def response(self, fC0, fCtot, lMin=1., lMax=1.e5, test=False, cache=None):
   ...
   def forecastN0KappaPSH(self, fC0, fCtot, lMin=1., lMax=1.e5, test=False, cache=None):
      ...
      # Standard reconstruction noise
      r = self.computeResponseFFT(fC0, fCtot, lMin, lMax, test, cache)
      ns = self.computeSNormalizationFFT(fCtot, lMin, lMax, test, cache)
