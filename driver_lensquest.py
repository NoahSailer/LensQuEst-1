...
forCtotal = lambda l: cmb.flensedTT(l)+cmb.fdetectorNoise(l)+cmb.fCIB(l)+cmb.ftSZ(l)+cmb.fkSZ(l)+cmb.fradioPoisson(l)
#
...
#
ax.loglog(L,cmb.fdetectorNoise(L),'k--',label='detector noise')
...
ax.set_xlabel(r'$\ell$')
ax.set_ylabel(r'$C_\ell\hspace{1mm} [\mu K^2]$')
...
#NssB = fNssBCmb_fft(L)
#
ax.loglog(L, Ns, c=cS, label=r'Shear E')
ax.loglog(L, Nd, c=cD, label=r'Mag.')
ax.loglog(L, Npsh, c=cPSH, label=r'PSH')
...
#print "Cross-power: kappa_rec x kappa_true"
#lCen, Cl, sCl = baseMap.crossPowerSpectrum(pshCmbFourier, kCmbFourier,theory=[p2d_cmblens.fPinterp,fNpshCmb_fft], plot=True, save=False)
